...
from config import get_color, get_image, convert_big_value, draw_text, shit_to_name, get_top10
...
class SingleRow:
    ...
    def create_image(self, coin):
        ...
        if coin:
            ...
            # Graph
            img_rq = requests.get(get_image(name))
            graph = Image.open(BytesIO(img_rq.content)).convert('RGBA')
            # graph = graph.resize((100, 50))
            ...
            # Name
            draw_text(draw=draw,
                      pos=(self.dx, self.height / 2 - self.m_font / 2 - 2),
                      text=name,
                      size=self.m_font,
                      font='RMM.ttf')
